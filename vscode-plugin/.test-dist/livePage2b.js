...
async function main() {
  ...
  const variants = [
    { m: "bug", f: "browse", productid: "34", branch: "all", browseType: "bySearch", pageID: "2", recPerPage: "20", recTotal: "22" },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "bySearch", pageID: "2", recPerPage: "20", recTotal: "22" },
    { m: "bug", f: "browse", productid: "34", branch: "all", browseType: "bySearch", param: "0", orderBy: "id_desc", pageID: "2", recPerPage: "20", recTotal: "22" }
  ];
  const out = [];
  for (const params of variants) {
    const page = await client.getBugListText(params, void 0, true);
    const bugs = parseBugList2(page.html);
    out.push({ params, count: bugs.length, ids: bugs.map((bug) => bug.id), pager: parseBugListPager2(page.html), rowHints: ["184035", "184034"].map((id) => [id, page.html.includes(id)]) });
  }
