...
__export(index_exports, {
  DEFAULT_ZENTAO_SERVER_URL: () => DEFAULT_ZENTAO_SERVER_URL,
  LoginExpiredError: () => LoginExpiredError,
  ZenTaoClient: () => ZenTaoClient,
  __zentaoParserTestInternals: () => __zentaoParserTestInternals,
  buildBatchBugFixPrompt: () => buildBatchBugFixPrompt,
  buildBugFixPrompt: () => buildBugFixPrompt,
  describeErrorChain: () => describeErrorChain,
...
function buildBatchBugFixPrompt(bugs) {
  const body = bugs.map((bug, index) => {
    const reproduceImages = (bug.promptImages?.length ? bug.promptImages : extractReproduceStepImages(bug.reproduceStepsHtml)).slice(0, maxImageAttachments);
    const reproduceImageText = reproduceImages.length ? reproduceImages.map((url, imageIndex) => `  - \u56FE\u7247${imageIndex + 1}\uFF1A${url}`).join("\n") : `  ${emptyText}`;
    const bugDescription = textOrFallback(bug.description, bug.title);
    const reproduceText = textOrFallback(htmlToPromptText(bug.reproduceStepsHtml), bug.reproduceSteps);
    const expectedText = textOrFallback(htmlToPromptText(bug.expectedResultHtml), bug.expectedResult);
    const actualText = textOrFallback(bug.actualResult);
...
function formatBugMetadata(bug) {
  return [
    ...
    `- \u5DF2\u786E\u8BA4\uFF1A${bug.confirmed === void 0 ? emptyText : bug.confirmed ? "\u662F" : "\u5426"}`
  ].join("\n");
}
function formatAttachments(bug, prefix = "") {
...
var DEFAULT_ZENTAO_SERVER_URL = "http://your-zentao-server/";
var PLACEHOLDER_SERVER_URLS = /* @__PURE__ */ new Set([
  "",
...
function isBySearchFallbackParams(params) {
...
function hasSameBugIds(left, right) {
...
function resolveServerUrl(configured) {
...
function extractImagesFromHtml(html) {
  ...
  for (const match of html.matchAll(/<img\b[^>]*>/gi)) {
    const tag = match[0] ?? "";
    const url = readImageAttr2(tag, "data-original-src") || readImageAttr2(tag, "src");
    if (url) urls.push(url);
  }
  return urls;
}
function readImageAttr2(tag, name) {
  return tag.match(new RegExp(`\\b${name}=["']([^"']+)["']`, "i"))?.[1];
}
