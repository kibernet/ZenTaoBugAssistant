...
async function main() {
  ...
  const list = await client.listBugs({ projectId: "34", assigneeScope: "all", teamMembers: [] });
  console.log(JSON.stringify({ firstCount: bugs.length, pager, listCount: list.length, ids: list.slice(0, 10).map((bug) => bug.id) }, null, 2));
}
main().catch((error) => {
