...
async function main() {
  ...
  const yaoProjects = projects.filter((project) => project.name.includes("????"));
  const samples = [];
  for (const project of yaoProjects.slice(0, 8)) {
    try {
      const bugs = await client.listBugs({ projectId: project.id, assigneeScope: "all", teamMembers: [] });
      samples.push({ projectId: project.id, label: project.name, count: bugs.length, ids: bugs.slice(0, 8).map((bug) => bug.id), titles: bugs.slice(0, 3).map((bug) => bug.title) });
    } catch (error) {
      samples.push({ projectId: project.id, label: project.name, error: error instanceof Error ? error.message : String(error) });
    }
  }
  try {
    const bugs34 = await client.listBugs({ projectId: "34", assigneeScope: "all", teamMembers: [] });
    samples.push({ projectId: "34", label: "direct productID 34", count: bugs34.length, ids: bugs34.slice(0, 8).map((bug) => bug.id), titles: bugs34.slice(0, 3).map((bug) => bug.title) });
  } catch (error) {
    samples.push({ projectId: "34", label: "direct productID 34", error: error instanceof Error ? error.message : String(error) });
  }
  console.log(JSON.stringify({ projectCount: projects.length, yaoProjects, samples }, null, 2));
}
main().catch((error) => {
