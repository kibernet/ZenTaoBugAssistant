...
// livePage2.ts
var { parseBugList: parseBugList2, parseBugListPager: parseBugListPager2 } = __zentaoParserTestInternals;
async function main() {
  ...
  const firstParams = { m: "bug", f: "browse", productid: "34", branch: "all", browseType: "bySearch" };
  const first = await client.getBugListText(firstParams, void 0);
  const firstBugs = parseBugList2(first.html);
  const pager = parseBugListPager2(first.html);
  const probes = [];
  const pageVariants = [
    { ...firstParams, pageID: "2", recPerPage: "20", recTotal: String(pager?.recTotal ?? 22) },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "bySearch", pageID: "2", recPerPage: "20", recTotal: String(pager?.recTotal ?? 22) },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "", pageID: "2", recPerPage: "20", recTotal: "22" },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "", pageID: "2", recPerPage: "20", recTotal: String(pager?.recTotal ?? 22) }
  ];
  for (const params of pageVariants) {
    try {
      const page = await client.getBugListText(params, void 0, first.ajax);
      const bugs = parseBugList2(page.html);
      probes.push({ params, ajax: page.ajax, count: bugs.length, ids: bugs.map((bug) => bug.id), pager: parseBugListPager2(page.html), textHints: { has184035: page.html.includes("184035"), has184034: page.html.includes("184034"), title: page.html.match(/<title>([\s\S]*?)<\/title>/i)?.[1] } });
    } catch (error) {
      probes.push({ params, error: error instanceof Error ? error.message : String(error) });
    }
  }
  console.log(JSON.stringify({ first: { ajax: first.ajax, count: firstBugs.length, ids: firstBugs.slice(0, 5).map((bug) => bug.id), pager }, probes }, null, 2));
}
main().catch((error) => {
