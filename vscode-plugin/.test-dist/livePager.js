...
async function main() {
  ...
  const params = { m: "bug", f: "browse", productid: "34", branch: "all", browseType: "bySearch", pageID: "1", recPerPage: "20" };
  const page = await client.getBugListText(params, void 0);
  const bugs = parseBugList2(page.html);
  const pager = parseBugListPager2(page.html);
  console.log(JSON.stringify({ ajax: page.ajax, count: bugs.length, ids: bugs.slice(0, 5).map((bug) => bug.id), pager }, null, 2));
}
main().catch((error) => {
