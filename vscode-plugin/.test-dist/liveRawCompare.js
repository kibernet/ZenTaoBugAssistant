...
// liveRawCompare.ts
var { parseBugList: parseBugList2 } = __zentaoParserTestInternals;
async function main() {
  ...
  const params = { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "", recTotal: "22", recPerPage: "20", pageID: "1" };
  const ajax = await client.getText("index.php", params, true);
  const plain = await client.getText("index.php", params, false);
  const summarize = (name, html) => ({ name, length: html.length, count: parseBugList2(html).length, has184055: html.includes("184055"), hasAppIframe: html.includes("appIframe"), hasModuleSetting: html.includes("??????????"), title: html.match(/<title>([\s\S]*?)<\/title>/i)?.[1], preview: html.replace(/\s+/g, " ").slice(0, 300) });
  console.log(JSON.stringify([summarize("ajax", ajax), summarize("plain", plain)], null, 2));
}
main().catch((error) => {
