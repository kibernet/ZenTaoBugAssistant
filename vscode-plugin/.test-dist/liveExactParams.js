...
async function main() {
  ...
  const variants = [
    ...
    { m: "bug", f: "browse", productid: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "" },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "id_desc" },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "", recTotal: "22", recPerPage: "20", pageID: "1" },
    { m: "bug", f: "browse", productID: "34", branch: "all", browseType: "unclosed", param: "0", orderBy: "", recTotal: "22", recPerPage: "20", pageID: "2" }
  ];
  const out = [];
  for (const params of variants) {
    try {
      const page = await client.getBugListText(params, void 0);
      const bugs = parseBugList2(page.html);
      out.push({ params, ajax: page.ajax, count: bugs.length, ids: bugs.slice(0, 25).map((bug) => bug.id), pager: parseBugListPager2(page.html), title: page.html.match(/<title>([\s\S]*?)<\/title>/i)?.[1], hints: { has184055: page.html.includes("184055"), has184034: page.html.includes("184034"), has183099: page.html.includes("183099") } });
    } catch (error) {
      out.push({ params, error: error instanceof Error ? error.message : String(error) });
    }
  }
